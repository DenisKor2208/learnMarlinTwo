...
$found_user = get_user_by_email_or_id("two_person", $entered_email, null);
...
if(empty($entered_password) and empty($found_user['email'])) {              //не меняли пароль, но изменили емаил
    ...
    edit_credentials("two_person", $open_user_id, null, $entered_password);
}else {                                                                     //изменили емаил и ввели пароль
    edit_credentials("two_person", $open_user_id, $entered_email, $entered_password);
    if(is_author($_SESSION['id'], $open_user_id)){$_SESSION['email'] = $entered_email;}
}
